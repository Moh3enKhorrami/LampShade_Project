        ...
        const product = `<div class="single-cart-item">
                                    ...
                                    <div class="content">
                                        <p class="product-title">
                                            <a href="single-product.html">Name: ${x.name}</a>
                                        </p>
                                        <p class="count">Count: ${x.count}</p>
                                        <p class="count">Price: ${x.unitPrice}</p>
                                    </div>
                                </div>`;
...
function changeCartItemCount(id, totalId, count) {
    ...
    const settings = {
        "url": "https://localhost:7016/api/Inventory",
        "method": "POST",
        "timeout": 0,
        "headers": {
            ...
            if ($(`#${id}`).length == 0) {
                warningsDiv.append(`
                    <div class="alert alert-warning" id="${id}">
                        <i class="fa fa-warning"></i> article
                        <strong>${data.productName}</strong>
                        There is less than the requested quantity in stock.
                    </div>
                `);
            }
        } else {
